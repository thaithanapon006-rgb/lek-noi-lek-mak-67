...
/**
 * โครงสร้างของแต่ละบท (รูปแบบ "สไลด์" — ใช้กับบทที่ 2-6 ในปัจจุบัน)
 * id           : เลขบท (unique)
 * title        : ชื่อบท
 ...
 * locked       : true = ยังไม่เปิดให้เล่น แสดงเป็นการ์ด "เร็วๆ นี้" ในหน้ารวมบทเรียน
 * slides       : เนื้อหาสื่อการสอน (อาร์เรย์ของสไลด์) ต้องดูให้ครบก่อนเล่นเกมได้
 * game         : ค่าคอนฟิกมินิเกมของบทนี้
 *   type       : ชนิดเกม ('compare' | 'count' | 'addition' | 'subtraction' | 'order' | 'wordproblem' | 'quiz')
 *   questions  : คำถามของเกม (โครงสร้างขึ้นกับ type ดูรายละเอียดใน games.js)
 *
 * โครงสร้างของบทแบบ "วิดีโอ" (ใช้กับบทที่ 1 ในปัจจุบัน)
 * type          : "video"
 * chapterTitle  : เช่น "บทที่ 1 ง่ายมาก"
 * missionTitle  : เช่น "ภารกิจกู้คืนอุโมงค์ปู"
 * videoUrl      : ลิงก์ YouTube เต็ม
 * videoId       : รหัสวิดีโอ (ดึงมาจาก videoUrl)
 * videoTitle    : ชื่อวิดีโอสำรอง (ใช้ตอนที่ดึงชื่อจริงจาก YouTube ไม่สำเร็จ)
 * imageCredit   : ข้อความเครดิตภาพ/สื่อ แสดงตัวเล็กจางใต้วิดีโอ ห้ามลบ
 * game          : เหมือนบทแบบสไลด์ทุกประการ (ใช้เอนจินเดิม)
 */
const LESSONS = [
  {
    id: 1,
    type: "video",
    chapterTitle: "บทที่ 1 ง่ายมาก",
    missionTitle: "ภารกิจกู้คืนอุโมงค์ปู",
    videoUrl: "https://www.youtube.com/watch?v=rg43LERjKvE",
    videoId: "rg43LERjKvE",
    // ชื่อวิดีโอจริงจะถูกดึงจาก YouTube โดยอัตโนมัติ (ดู js/youtube.js)
    // ค่านี้ใช้แสดงชั่วคราวระหว่างโหลด หรือใช้สำรองถ้าดึงไม่สำเร็จ
    // ถ้าทราบชื่อวิดีโอที่ถูกต้องแน่นอนแล้ว ให้แก้ไขข้อความนี้ให้ตรงกับชื่อจริงได้เลย
    ...
    game: {
      ...
      questions: [
        {
          icon: "🦀",
          title: "ข้อ 1 — นับอุโมงค์",
          prompt: "ในตอนแรก เกาะมีอุโมงค์ปู 1 แห่ง สำหรับให้ปูเดินผ่าน ต่อมาอุโมงค์ถูกดินเหนียวปิดไป 1 แห่ง ตอนนี้เหลืออุโมงค์ที่ปูใช้เดินได้กี่แห่ง?",
          options: [
            { label: "🔵 0 แห่ง", correct: true },
            { label: "🟢 1 แห่ง" },
            { label: "🟡 2 แห่ง" },
            { label: "🔴 3 แห่ง" }
          ]
        ...
        {
          icon: "🧱",
          title: "ข้อ 2 — ช่วยเปิดทางให้ปู",
          prompt: "อุโมงค์มีดินเหนียวขวางอยู่ 3 จุด ปาปิยองเจาะออกไปแล้ว 1 จุด ยังเหลือดินเหนียวอีกกี่จุด?",
          options: [
            { label: "🔵 1 จุด" },
            { label: "🟢 2 จุด", correct: true },
            { label: "🟡 3 จุด" },
            { label: "🔴 4 จุด" }
          ]
        },
        {
          icon: "👟",
          title: "ข้อ 3 — หลักฐานของแอนเทล",
          prompt: "บริเวณอุโมงค์พบรองเท้า 1 ข้าง ถ้าเราพบรองเท้าของแอนเทล 1 ข้าง และพบแอนเทล 1 คน เราสามารถจับคู่ได้กี่คู่?",
          options: [
            { label: "🔵 0 คู่" },
            { label: "🟢 1 คู่", correct: true },
            { label: "🟡 2 คู่" },
            { label: "🔴 3 คู่" }
          ]
        },
        {
          icon: "🦀",
          title: "ข้อ 4 — ปูเพิ่มขึ้นเท่าไร?",
          prompt: "ก่อนเกิดเหตุ ปูเดินทางผ่านอุโมงค์ เมื่ออุโมงค์ถูกปิด ปูจึงเปลี่ยนเส้นทางขึ้นมาบนถนน ถ้าเปรียบเทียบ “จำนวนเส้นทางที่ปูใช้ได้” จากเดิม 1 ทาง เมื่ออุโมงค์ถูกปิดเหลือ 0 ทาง จำนวนเส้นทางลดลงกี่ทาง?",
          options: [
            { label: "🔵 0 ทาง" },
            { label: "🟢 1 ทาง", correct: true },
            { label: "🟡 2 ทาง" },
            { label: "🔴 3 ทาง" }
          ]
        },
        {
          icon: "🚨",
          title: "ข้อ 5 — ภารกิจกู้ภัย",
          prompt: "ปาปิยองต้องเจาะดินเหนียว 3 ครั้ง เพื่อเปิดทางให้อุโมงค์ เขาเจาะไปแล้ว 2 ครั้ง ต้องเจาะอีกกี่ครั้งจึงจะครบ 3 ครั้ง?",
          options: [
            { label: "🔵 1 ครั้ง", correct: true },
            { label: "🟢 2 ครั้ง" },
            { label: "🟡 3 ครั้ง" },
            { label: "🔴 4 ครั้ง" }
          ]
